...
import { MapPin, Calendar, Compass, Shield, Search, SlidersHorizontal, ChevronLeft, ChevronRight, Users, Car, Star, ArrowUpDown, Loader2 } from 'lucide-react';
...
interface TripsListClientProps {
  /**
   * Server-side fetched first page of trips. Rendered immediately —
   * no blank screen or spinner on initial visit.
   */
  initialTrips: Trip[];
  initialTotalPages: number;
}

export default function TripsListClient({ initialTrips, initialTotalPages }: TripsListClientProps) {
  // Filter state
  ...
  // Data — seeded from server-fetched initial data (no loading state on first render)
  ...
  const fetchTrips = (currentPage: number, overrides?: {
    destination?: string; budget?: string; date?: string; vehicle?: string;
    language?: string; tripType?: string; difficulty?: string; seats?: number;
    genderPref?: string; sortBy?: string;
  }) => {
    startTransition(async () => {
      setErrorMsg(null);
      const res = await listTripsAction(
        {
          destination: (overrides?.destination ?? destination) || undefined,
          budget: ((overrides?.budget ?? budget) || undefined) as BudgetRange | undefined,
          date: (overrides?.date ?? date) || undefined,
          vehicle: (overrides?.vehicle ?? vehicle) || undefined,
          language: (overrides?.language ?? language) || undefined,
          tripType: ((overrides?.tripType ?? tripType) || undefined) as TripType | undefined,
          difficulty: ((overrides?.difficulty ?? difficulty) || undefined) as Difficulty | undefined,
          seats: (overrides?.seats ?? seats) > 0 ? (overrides?.seats ?? seats) : undefined,
          genderPref: ((overrides?.genderPref ?? genderPref) || undefined) as any,
        },
        currentPage,
        6,
        (overrides?.sortBy ?? sortBy) as any
      );
  ...
  const handleResetFilters = () => {
    ...
    setSortBy('NEAREST');
    setPage(1);
    // Pass reset values directly to avoid setState race conditions
    fetchTrips(1, {
      destination: '', budget: '', date: '', vehicle: '', language: '',
      tripType: '', difficulty: '', seats: 0, genderPref: '', sortBy: 'NEAREST',
    });
  };

  const FilterForm = ({ onSubmit }: { onSubmit: (e: React.FormEvent) => void }) => (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        <select value={genderPref} onChange={(e) => setGenderPref(e.target.value as any)}
          className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none">
      ...
      <div className="space-y-1.5">
        ...
        <input type="number" min="0" value={seats || ''} onChange={(e) => setSeats(Number(e.target.value))}
          placeholder="e.g. 2"
          className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none" />
      </div>

      <button type="submit" disabled={isPending}
        className="w-full bg-foreground hover:bg-foreground/90 text-background font-bold py-2.5 rounded-xl text-sm transition-colors shadow-sm disabled:opacity-60 flex items-center justify-center gap-2">
  ...
  return (
    <>
      {/* Sort bar + mobile filter toggle + post trip CTA */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-card border border-border px-3.5 py-2 rounded-full text-xs font-semibold">
            ...
            <select value={sortBy} onChange={(e) => { setSortBy(e.target.value as any); setPage(1); fetchTrips(1, { sortBy: e.target.value }); }}
          ...
          <Link href="/trips/map"
            className="inline-flex items-center gap-1.5 border border-border bg-card hover:bg-secondary text-foreground px-4.5 py-2.5 rounded-full text-sm font-semibold transition-colors text-center shadow-sm"
          >
            <Compass className="h-4.5 w-4.5 text-primary" /> Map View
          </Link>
        ...
        <div className="lg:col-span-3 space-y-8">
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {(trip as any).compatibility && (
                            <span className={`text-xs font-extrabold px-2 py-0.5 rounded-full border flex items-center gap-1.5 shadow-sm ${
                              (trip as any).compatibility.score >= 80
                                ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                                : (trip as any).compatibility.score >= 50
                                ? 'bg-amber-50 text-amber-700 border-amber-200'
                                : 'bg-slate-50 text-slate-700 border-slate-200'
                            }`}>
                              <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse" />
                              {(trip as any).compatibility.score}% Match
                            </span>
                          )}
                        </div>
                      ...
                      {/* Match explanation notice */}
                      {(trip as any).compatibility && (
                        <div className="mt-4 p-3 bg-secondary/35 rounded-xl border border-border/50 text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5">
                          <span className="text-primary font-extrabold">{(trip as any).compatibility.score}% Match</span>
                          <span>•</span>
                          <span>{(trip as any).compatibility.reasons.join(' & ')}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="border-t border-border mt-6 pt-4 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          ...
                          {trip.owner?.profile?.rating && trip.owner.profile.rating > 0 && (
          ...
          {totalPages > 1 && !isPending && (
            <div className="flex items-center justify-center gap-4 pt-6 border-t border-border">
              <button onClick={() => { const p = Math.max(1, page - 1); setPage(p); fetchTrips(p); }}
                disabled={page === 1}
              ...
              <button onClick={() => { const p = Math.min(totalPages, page + 1); setPage(p); fetchTrips(p); }}
        ...
        <div className="fixed inset-0 z-50 lg:hidden flex justify-end">
          ...
          <div className="relative w-full max-w-sm bg-card border-l border-border h-full p-6 shadow-xl overflow-y-auto animate-in slide-in-from-right duration-200">
            ...
            <button type="button" onClick={handleResetFilters}
              className="w-full mt-3 border border-border hover:bg-secondary py-2.5 rounded-xl text-sm font-semibold transition-colors">
              Reset All
            </button>
          </div>
        </div>
